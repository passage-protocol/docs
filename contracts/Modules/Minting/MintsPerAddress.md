...
```solidity
function canMint(address minter, uint256 value, uint256[], uint256[] mintAmounts, bytes32[], bytes) external nonpayable returns (uint256)
```
...
```solidity
function hasAddressMinted(address) external view returns (bool)
```
...
| _minter | address | The address of the wallet or contract that can call canMint (passport/LL) |
| data | bytes | encoded uint256 maxClaim - max amount of tokens per address &amp; uint256 mintPrice - price per token in wei |
