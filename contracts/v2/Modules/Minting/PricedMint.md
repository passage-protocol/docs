...
```solidity
function canMint(address, uint256 value, uint256[], uint256[] mintAmounts, bytes32[], bytes) external view returns (uint256)
```

Mint Passport token(s) to caller

*Must first set isActive to true*
...
| mintAmounts | uint256[] | uint256[] the first element in mintAmounts specifies the amount to mint |
...
| data | bytes | encoded unint256 maxClaim - maximum amount of tokens per claim &amp; mintPrice - price per token in wei |
